...
void main() {
  ...
  group('PredictorPageManager drafts', () {
    ...
    Future<(ProviderContainer, PredictorPageManager)> buildLoaded({
      ...
      final container = ProviderContainer(
        overrides: [
          predictorPageManagerProvider.overrideWith((ref) {
            return PredictorPageManager(
      ...
      final manager = container.read(predictorPageManagerProvider);
      await manager.load();
    ...
    test('initial draft follows championship order', () async {
      ...
      final state = container.read(predictorPageStateHolderProvider);
    ...
    test('moveDraftTo swaps without shifting others', () async {
      final (container, manager) = await buildLoaded(championshipIds: ['a', 'b', 'c', 'd']);
      addTearDown(container.dispose);
      // a b c d → swap index 0 with 2 → c b a d
      await manager.moveDraftTo(fromIndex: 0, toIndex: 2);
      expect(container.read(predictorPageStateHolderProvider).draftQualifyingOrder, ['c', 'b', 'a', 'd']);
    });

    test('copyQualifyingToRace copies current quali draft', () async {
      final (container, manager) = await buildLoaded(championshipIds: ['a', 'b', 'c']);
      addTearDown(container.dispose);
      await manager.moveDraftTo(fromIndex: 0, toIndex: 2);
      manager.selectGrid(PredictorGridKind.race);
      await manager.copyQualifyingToRace();
      final state = container.read(predictorPageStateHolderProvider);
    ...
    test('reorderDraft inserts with shift', () async {
      final (container, manager) = await buildLoaded(championshipIds: ['a', 'b', 'c']);
      addTearDown(container.dispose);
      await manager.reorderDraft(oldIndex: 0, newIndex: 2);
      // remove a → [b,c], insert at 2 → [b,c,a]
      expect(container.read(predictorPageStateHolderProvider).draftQualifyingOrder, ['b', 'c', 'a']);
    });
