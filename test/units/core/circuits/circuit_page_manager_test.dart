...
void main() {
  ...
  ProviderContainer buildContainer({
    ...
    return ProviderContainer(
      overrides: [
        ...
        circuitPageManagerProvider(circuit).overrideWith((ref) {
  ...
  group('CircuitPageManager', () {
    test('loadAll sets winners, photo and stats', () async {
      final container = buildContainer();
      addTearDown(container.dispose);

      await container.read(circuitPageManagerProvider(circuit)).loadAll();
      final state = container.read(circuitPageStateHolderProvider(circuit));
    ...
    test('winners failure sets screenError', () async {
      ...
      await container.read(circuitPageManagerProvider(circuit)).loadWinners();
      final state = container.read(circuitPageStateHolderProvider(circuit));
    ...
    test('photo loading flag flips while fetch is in flight', () async {
      ...
      final manager = container.read(circuitPageManagerProvider(circuit));
      final pending = manager.loadPhoto();
      expect(container.read(circuitPageStateHolderProvider(circuit)).isPhotoLoading, isTrue);
    ...
    test('refreshAll reloads winners', () async {
      ...
      await container.read(circuitPageManagerProvider(circuit)).refreshAll();
      expect(calls, 1);
      expect(container.read(circuitPageStateHolderProvider(circuit)).isLoaded, isTrue);
    });
  });
}
