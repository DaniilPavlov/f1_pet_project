...
/// Загрузка списков конструкторов Jolpica (с кэшем).
abstract class ConstructorLookup {
  static List<ConstructorModel>? _currentCache;
  static Future<List<ConstructorModel>>? _currentInFlight;

  static List<ConstructorModel>? _allCache;
  static Future<List<ConstructorModel>>? _allInFlight;

  /// Конструкторы текущего сезона.
  static Future<List<ConstructorModel>> loadCurrentConstructors() => _currentConstructors();

  /// Все конструкторы (пагинация).
  static Future<List<ConstructorModel>> loadAllConstructors() => _allConstructors();

  static Future<List<ConstructorModel>> _currentConstructors() async {
  ...
  static Future<List<ConstructorModel>> _allConstructors() async {
    if (_allCache != null) {
      return _allCache!;
    }
  ...
  static Future<List<ConstructorModel>> _fetchCurrentConstructors() async {
    try {
      ...
      debugPrint('ConstructorLookup.currentConstructors failed: $error\n$stackTrace');
      return _currentCache ?? const [];
    }
  }

  static Future<List<ConstructorModel>> _fetchAllConstructors() async {
    try {
      const pageSize = 100;
      ...
      debugPrint('ConstructorLookup.allConstructors failed: $error\n$stackTrace');
      return _allCache ?? const [];
    }
  }

  static List<ConstructorModel> _parseConstructors(Object? mrData) {
    if (mrData is! Map) {
      return const [];
    }
