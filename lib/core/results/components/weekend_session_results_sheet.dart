...
class _WeekendSessionResultsSheetState extends State<WeekendSessionResultsSheet> {
  ...
  Future<void> _openDriver(EspnScoreboardResultEntry entry) async {
    ...
    try {
      ...
      final driver = await DriverLookup.findByDisplayName(entry.displayName);
      if (!mounted) {
        return;
      }
      if (driver == null) {
        await Fluttertoast.showToast(
          msg: context.l10n.driverNotFound,
    ...
    return SizedBox(
      ...
      child: DefaultBottomSheet(
        body: Column(
          ...
          children: [
            Text(
              context.l10n.weekendSessionResultsTitle(session.abbreviation),
              style: AppStyles.h2,
            ),
