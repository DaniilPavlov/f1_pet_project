...
class _DriverScreenState extends ConsumerState<DriverScreen> {
  late final DriverPageArgs _args;

  @override
  void initState() {
    super.initState();
    _args = DriverPageArgs(driver: widget.driver, currentConstructors: widget.currentConstructors);
    final fullName = '${widget.driver.givenName} ${widget.driver.familyName}';
    Future.microtask(() {
      ref.read(analyticsGatewayProvider).log(
            DriverOpened(driverId: widget.driver.driverId, driverName: fullName),
          );
      return ref.read(driverPageManagerProvider(_args)).loadAll();
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    final currentConstructors = widget.currentConstructors;
    final fullName = '${driver.givenName} ${driver.familyName}';
    final viewModel = ref.watch(driverPageStateHolderProvider(_args));
    final manager = ref.watch(driverPageManagerProvider(_args));
    final stats = viewModel.careerStats.value;
      ...
      body: SafeArea(
        child: Builder(
          builder: (context) {
            ...
            if (!viewModel.isLoaded || stats == null) {
              return const CareerScreenShimmer();
            }

            final espn = viewModel.espnCardData;
                      ...
                      child: Column(
                        ...
                        children: [
                          EspnDriverPhoto(
                            ...
                            isLoading: viewModel.isEspnLoading,
