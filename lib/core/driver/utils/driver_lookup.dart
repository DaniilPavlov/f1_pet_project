...
/// Поиск пилота Jolpica по displayName из ESPN (с кэшем списков).
abstract class DriverLookup {
  static List<DriverModel>? _currentCache;
  static Future<List<DriverModel>>? _currentInFlight;

  static List<DriverModel>? _allCache;
  static Future<List<DriverModel>>? _allInFlight;

  /// Список пилотов текущего сезона (с кэшем).
  static Future<List<DriverModel>> loadCurrentDrivers() => _currentDrivers();

  /// Все пилоты Jolpica (пагинация, с кэшем).
  static Future<List<DriverModel>> loadAllDrivers() => _allDrivers();

  /// Ищет пилота текущего сезона по имени вида `Kimi Antonelli` / `Max Verstappen`.
  static Future<DriverModel?> findByDisplayName(String displayName) async {
  ...
  static Future<List<DriverModel>> _currentDrivers() async {
  ...
  static Future<List<DriverModel>> _allDrivers() async {
    if (_allCache != null) {
      return _allCache!;
    }
  ...
  static Future<List<DriverModel>> _fetchCurrentDrivers() async {
    try {
      ...
      debugPrint('DriverLookup.currentDrivers failed: $error\n$stackTrace');
      return _currentCache ?? const [];
    }
  }

  static Future<List<DriverModel>> _fetchAllDrivers() async {
    try {
      const pageSize = 100;
      ...
      debugPrint('DriverLookup.allDrivers failed: $error\n$stackTrace');
      return _allCache ?? const [];
    }
  }

  static List<DriverModel> _parseDrivers(Object? mrData) {
    if (mrData is! Map) {
      return const [];
    }
  ...
  static DriverModel? _match(List<DriverModel> drivers, String displayName) {
