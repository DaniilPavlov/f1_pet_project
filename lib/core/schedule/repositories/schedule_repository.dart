...
///
/// Используется и экраном календаря, и сервисом напоминаний.
class ScheduleRepository {
  ...
  /// Возвращает расписание из дневного кэша или сети.
  ///
  /// Параллельные вызовы делят один in-flight запрос.
  ...
  Future<ScheduleLoadResult> _load({required bool forceRefresh}) async {
    ...
    try {
      final response = await ScheduleLoader.loadData();
      ...
      debugPrint('ScheduleRepository: fetch failed, fallback to cache: $error');
      if (cachedRaw != null) {
        return ScheduleLoadResult(
          schedule: ScheduleModel.fromJson(Map<String, dynamic>.from(jsonDecode(cachedRaw) as Map)),
          fetchedFromNetwork: false,
