...
class PredictorLeaderboardScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return PredictorAuthGate(
      child: Provider<PredictorLeaderboardController>(
        create: (context) => PredictorLeaderboardController(
          repository: context.read<PredictorLeaderboardRepository>(),
          year: year,
          myPoints: myPoints,
        )..load(),
        child: Scaffold(
          appBar: CustomAppBar(
            title: context.l10n.predictorLeaderboardTitle(year),
            onPop: () => context.router.maybePop(),
          ),
          body: SafeArea(
            child: Observer(
              builder: (context) {
                final controller = context.read<PredictorLeaderboardController>();
...
class _LeaderboardBody extends StatefulWidget {
  ...
  final PredictorLeaderboardController controller;
...
class _LeaderboardBodyState extends State<_LeaderboardBody> {
  ...
  void initState() {
    super.initState();
    _nicknameController = TextEditingController(text: widget.controller.nicknameDraft);
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Observer(
      builder: (context) {
        final entries = controller.rankedEntries;
        final myEntry = controller.myEntry;
        final showJoin = controller.showJoinForm;
              ...
              children: [
                if (showJoin) ...[
                  ...
                  CheckboxListTile(
                    ...
                    value: controller.optInAgreed,
                    onChanged: controller.isSaving
                        ? null
                        : (value) => controller.setOptInAgreed(value ?? false),
                    title: Text(
