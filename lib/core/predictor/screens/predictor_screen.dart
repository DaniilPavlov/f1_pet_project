...
class _PredictorAuthedScreenState extends ConsumerState<_PredictorAuthedScreen> {
  ...
  void initState() {
    super.initState();
    Future.microtask(() => ref.read(predictorScreenControllerProvider.notifier).load());
  }

  @override
  Widget build(BuildContext context) {
    final state = ref.watch(predictorScreenControllerProvider);
    final controller = ref.read(predictorScreenControllerProvider.notifier);
      ...
      body: SafeArea(
        child: Builder(
          builder: (context) {
            ...
            return _PredictorBody(state: state, controller: controller);
          },
        ),
      ),
    );
...
class _PredictorBody extends StatelessWidget {
  const _PredictorBody({required this.state, required this.controller});

  final PredictorScreenState state;
  ...
  Widget build(BuildContext context) {
    ...
    final history = state.historyWeekends;
    final archivedSeasons = state.archivedSeasonSummaries;
    final locked = state.isLocked;
    final selectedGrid = state.selectedGrid;
    final order = selectedGrid == PredictorGridKind.qualifying
        ? state.draftQualifyingOrder
        : state.draftRaceOrder;
    final byId = state.driversById;
    final constructorsById = state.constructorsByDriverId;
            ...
            SliverPadding(
              ...
              sliver: SliverToBoxAdapter(
                child: Column(
                  ...
                  children: [
                    Text(
                      context.l10n.predictorSeasonPoints(year, state.seasonTotalPoints),
                      style: AppStyles.h3,
                    ),
                    if (state.seasonYear != null) ...[
                      ...
                      BlackButton(
                        ...
                        onTap: () async {
                          final seasonYear = state.seasonYear!;
                          await context.router.push(
                            PredictorLeaderboardRoute(
                              year: seasonYear,
                              myPoints: state.seasonTotalPoints,
                            ),
                          );
                        },
                      ),
              ...
              SliverPadding(
                ...
                sliver: SliverToBoxAdapter(
                  child: PredictorWeekendHeader(
                    race: race,
                    isLocked: locked,
                    missingQualifyingTime: state.missingQualifyingTime,
                    lockCountdown: state.lockCountdown,
