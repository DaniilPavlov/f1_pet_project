...
class PredictorWeekendDetailScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final args = PredictorWeekendDetailPageArgs(season: season, weekend: weekend);
...
class _WeekendDetailAuthedScreenState extends ConsumerState<_WeekendDetailAuthedScreen> {
  ...
  void initState() {
    super.initState();
    Future.microtask(() => ref.read(predictorWeekendDetailPageManagerProvider(widget.args)).load());
  }

  @override
  Widget build(BuildContext context) {
    final viewModel = ref.watch(predictorWeekendDetailPageStateHolderProvider(widget.args));
    final manager = ref.watch(predictorWeekendDetailPageManagerProvider(widget.args));
        ...
        child: Builder(
          builder: (context) {
            if (!viewModel.allDataIsLoaded &&
                viewModel.qualifyingCompare.isLoading &&
                viewModel.raceCompare.isLoading) {
            ...
            final compare = viewModel.activeCompare;
                ...
                child: ListView(
                  ...
                  children: [
                    ...
                    SegmentedButton<PredictorDetailSession>(
                      ...
                      onSelectionChanged: (value) => manager.selectSession(value.first),
                    ),
                      ...
                      ...compare.rows.map(
                        (row) => Padding(
                          ...
                          child: PredictorComparisonTile(
                            row: row,
                            driversById: Map.of(viewModel.driversById),
