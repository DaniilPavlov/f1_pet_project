...
class PredictorLeaderboardScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final args = PredictorLeaderboardPageArgs(year: year, myPoints: myPoints);
...
class _LeaderboardAuthedScreenState extends ConsumerState<_LeaderboardAuthedScreen> {
  ...
  void initState() {
    super.initState();
    Future.microtask(() => ref.read(predictorLeaderboardPageManagerProvider(widget.args)).load());
  }

  @override
  Widget build(BuildContext context) {
    final viewModel = ref.watch(predictorLeaderboardPageStateHolderProvider(widget.args));
    final manager = ref.watch(predictorLeaderboardPageManagerProvider(widget.args));
...
class _LeaderboardBodyState extends ConsumerState<_LeaderboardBody> {
  ...
  void initState() {
    super.initState();
    final draft = ref.read(predictorLeaderboardPageStateHolderProvider(widget.args)).nicknameDraft;
  ...
  Widget build(BuildContext context) {
    final viewModel = ref.watch(predictorLeaderboardPageStateHolderProvider(widget.args));
    final manager = ref.watch(predictorLeaderboardPageManagerProvider(widget.args));
    final entries = viewModel.rankedEntries;
    final myEntry = manager.myEntry;
    final showJoin = viewModel.showJoinForm;
    final formError = predictorLeaderboardErrorMessage(
      context.l10n,
      viewModel.formErrorKey,
    );
          ...
          children: [
            if (showJoin) ...[
              ...
              CustomTextField(
                ...
                onChanged: manager.setNicknameDraft,
              ...
              CheckboxListTile(
                ...
                value: viewModel.optInAgreed,
                onChanged: viewModel.isSaving ? null : (value) => manager.setOptInAgreed(value ?? false),
                title: Text(
